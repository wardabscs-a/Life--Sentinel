...
exports.loginRemotelyStart = loginRemotelyStart;
exports.loginRemotelyComplete = loginRemotelyComplete;
...
async function loginRemotelyStart() {
    const authProxyClient = new apiv2.Client({
        urlPrefix: (0, api_1.authProxyOrigin)(),
        auth: false,
    });
...
async function loginRemotelyComplete(code, codeVerifier) {
    try {
        const tokens = await getTokensFromAuthorizationCode(code, `${(0, api_1.authProxyOrigin)()}/complete`, codeVerifier);
        void (0, track_1.trackGA4)("login", { method: "google_remote" });
        const decoded = jwt.decode(tokens.id_token, { json: true });
        if (!decoded || typeof decoded === "string") {
            throw new error_1.FirebaseError("Invalid ID token received.");
        }
        return {
            user: decoded,
            tokens: tokens,
