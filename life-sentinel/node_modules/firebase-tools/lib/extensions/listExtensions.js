...
const replacementRegistry_1 = require("./replacementRegistry");
async function listExtensions(projectId) {
    ...
    if (showReplacements) {
        [instances, registry] = await Promise.all([
            (0, extensionsApi_1.listInstances)(projectId),
            (0, replacementRegistry_1.getReplacementsRegistry)(),
        ]);
    }
    else {
        instances = await (0, extensionsApi_1.listInstances)(projectId);
    }
    ...
    sorted.forEach((instance) => {
        ...
        let replacementPackage;
        if (showReplacements && registry) {
            replacementPackage = (0, replacementRegistry_1.getReplacementPackageName)(extension, registry);
