...
exports.MAX_REQUEST_BODY_BYTES = void 0;
exports.isUrl = isUrl;
...
exports.MAX_REQUEST_BODY_BYTES = 32 * 1024 * 1024;
async function bufferRequestBody(req) {
    ...
    for await (const chunk of req) {
        ...
        total += buf.length;
        if (total > exports.MAX_REQUEST_BODY_BYTES) {
            throw new error_1.FirebaseError(`Request body exceeds the ${exports.MAX_REQUEST_BODY_BYTES} byte limit`, {
                status: 413,
            });
        }
        chunks.push(buf);
...
function simpleProxy(hostOrRequestHandler) {
    ...
    return async (originalReq, originalRes, next) => {
        ...
        originalRes.on("error", (err) => logger_1.logger.debug("Response stream error:", method, path, err));
        if (!method || !path) {
            originalRes.end();
            return;
        }
        ...
        if (typeof hostOrRequestHandler === "string") {
            ...
            if (!["GET", "HEAD"].includes(method)) {
                try {
                    ...
                    if (err instanceof error_1.FirebaseError && err.status === 413) {
                        originalRes.statusCode = 413;
                        originalRes.end();
                        return;
                    }
                    logger_1.logger.debug("Error buffering request body:", method, path, err);
