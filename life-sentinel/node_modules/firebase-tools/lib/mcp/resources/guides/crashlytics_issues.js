...
Follow these steps to fetch issues and prioritize them.
  ...
  1. Use the 'crashlytics_get_report' tool to fetch the 'topIssues' report.
    ...
    1b. If the user asks for crashes and it is a mobile app, then set the *issueErrorType* filter to *FATAL*.
...
exports.crashlytics_issues = (0, resource_1.resource)({
    ...
    description: "Guides the coding agent when working with Crashlytics issues, including prioritization rules and procedures for diagnosing and fixing issues. ",
}, async (uri) => {
    return {
