...
class Config {
    ...
    deleteProjectDir(p) {
        if (path.isAbsolute(p)) {
            throw new error_1.FirebaseError(`Sanity: deleteProjectDir() should not be called with an absolute path.`);
        }
        const resolvedPath = this.path(p);
        if (!fs.existsSync(resolvedPath)) {
            throw new error_1.FirebaseError(`Failed to delete project directory ${p}: directory doesn't exist.`);
        }
        try {
            fs.rmSync(resolvedPath, { recursive: true });
        }
        catch (err) {
            throw new error_1.FirebaseError(`Failed to delete project directory ${p}: ${err}`, {
    ...
    lsProjectDir(p) {
        const resolvedPath = this.path(p);
        if (!fs.existsSync(resolvedPath)) {
            throw new error_1.FirebaseError(`Failed to list files in project directory ${p}: directory doesn't exist.`);
        }
        try {
            return fs.readdirSync(resolvedPath, { withFileTypes: true });
        }
        catch (err) {
            throw new error_1.FirebaseError(`Failed to list files in project directory ${p}: ${err}`, {
                original: err instanceof Error ? err : undefined,
            });
        }
    }
