...
exports.disableServiceAndPoll = disableServiceAndPoll;
...
async function disableService(projectId, service) {
    try {
        const res = await exports.apiClient.post(`projects/${projectId}/services/${service}:disable`, {}, { headers: { "x-goog-user-project": `${projectId}` } });
        return res.body;
    }
    catch (err) {
        throw new error_1.FirebaseError(`Error disabling service ${service}.`, {
            original: err,
        });
    }
}
async function disableServiceAndPoll(projectId, service, loggingPrefix) {
    utils.logLabeledBullet(loggingPrefix, `disabling service ${(0, colorette_1.bold)(service)}...`);
